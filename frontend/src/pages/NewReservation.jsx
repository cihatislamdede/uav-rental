...
const NewReservation = () => {
  ...
  const fetchData = async (page = currentPage) => {
    const response = await axios.get(
      API_URL + `/uavs/?search=${search}&page=${page}`,
      {
        headers: {
  ...
  return (
    <div className="container mx-auto">
      <h2 className="text-3xl font-semibold text-center my-4">UAVs</h2>
            ...
            {data.results?.map((row) => (
              <tr key={row.id} className="border-t border-blue-200">
                ...
                <td className="p-2 flex flex-col">
                  {cookies.user.username === row.owner && (
                    <Link
                      to={`/uavs/${row.id}/edit`}
                  ...
                  <Link
                    to={`/uavs/${row.id}/reserve`}
                    className="bg-primary text-white rounded-md p-2 mt-1"
                  >
                    Reserve
                  </Link>
                </td>
